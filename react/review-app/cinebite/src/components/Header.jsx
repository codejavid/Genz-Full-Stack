...
const Header = () => {
    ...
    const LogoutButton = () => {
        ...
        const handleLogout = async() => {
         ...
         return (
            user && <button className='text-green-900 bg-white border-gray-300' onClick={handleLogout}>
                Logout
            </button>
         )
    ...
    return (
        <div>
            <nav className="bg-white w-full z-20 top-0 start-0 shadow-2xl">
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    <a href="https://flowbite.com/" className="flex items-center space-x-3 rtl:space-x-reverse">
                        <img src="https://flowbite.com/docs/images/logo.svg" className="h-7" alt="Flowbite Logo" />
                        <span className="self-center text-xl text-heading font-semibold whitespace-nowrap">Cinebite</span>
                    </a>
                    ...
                    <div className="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="navbar-search">
                        ...
                        <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-default rounded-base bg-neutral-secondary-soft md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-neutral-primary">

                            {
                                navLists.map((item) => (
                                    <li key={item.name}>
                                        <Link to={item.to} className="block py-2 px-3 bg-brand rounded md:bg-transparent md:text-fg-brand md:p-0" aria-current="page">{item.name}</Link>
                                    </li>
